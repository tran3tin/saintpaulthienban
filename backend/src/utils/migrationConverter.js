...
function convertMySQLToPostgreSQL(mysqlQuery) {
  ...
  pgQuery = pgQuery.replace(/INT\s+UNSIGNED\s+AUTO_INCREMENT\s+PRIMARY\s+KEY/gi, 'SERIAL PRIMARY KEY');
  pgQuery = pgQuery.replace(/BIGINT\s+UNSIGNED\s+AUTO_INCREMENT\s+PRIMARY\s+KEY/gi, 'BIGSERIAL PRIMARY KEY');
